...
This must be completed from \emph{inspected data}, not from documentation.
...
\midrule
Surface pressure field name                & & & \\
Wall-shear field name(s)                   & & & \\
Pressure units (Pa or $\Cp$?)              & & & \\
\textbf{Variable order in the array}       & & & \\
\textbf{Streamwise axis (must be $x$ --- \cref{rem:streamwise})} & & & \\
Inlet velocity $U_\infty$                  & & & \\
Air density $\rho$                         & & & \\
Frontal area $A$ (given, or computed how?) & & & \\
Body length scale $L$                      & & & \\
Reference area behind \emph{their} published $\Cd$ & & & \\
Fields on points or on cells?              & & & \\
\bottomrule
\end{tabular}
\end{center}

\begin{criterion}[Phase 0]
\label{crit:p0}
The repository structure of \cref{sec:structure} exists, and its \texttt{external/}
section has been \emph{verified against the actual clone} and corrected; the upstream
commit hash is recorded; the compatibility table is fully populated from inspected data;
the canonical output space (\cref{def:canonical}) is committed in writing; and the
frontal-area convention for each body is stated in one sentence.
\end{criterion}
...
STATUS: completed through Phase ___
LAST RESULT: ___
BLOCKER: ___
UPSTREAM COMMIT: physicsnemo @ ___
\end{lstlisting}
